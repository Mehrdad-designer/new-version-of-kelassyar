...
export default function ClassCard({ data }: ClassCardProps) {
    ...
    const getLevelColor = (level: string) => {
        switch (level) {
            case 'مقدماتی': return 'bg-green-100 text-green-700';
            case 'متوسط': return 'bg-blue-100 text-blue-700';
            case 'پیشرفته': return 'bg-purple-100 text-purple-700';
        ...
        <div className="group bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute bottom-3 right-3">
                    <span className={`px-3 py-1 ${getLevelColor(data.level)} text-xs font-bold rounded-full`}>
                        {data.level}
                    </span>
                </div>
            ...
            <div className="p-5">
                ...
                <div className="flex items-center gap-2 mb-4 p-2 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-xs text-gray-600 line-clamp-1">{data.instructor.name}</p>
                        <div className="flex items-center gap-1">
                            {renderStars(data.instructor.rating)}
                            <span className="text-xs text-gray-500 mr-1">{data.instructor.rating}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 py-3 border-y border-gray-100">
                    <div className="text-center">
                        <div className="text-purple-600 font-bold">{data.sessionsCount}</div>
                ...
                <div className="mb-4">
                    <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                        <span>ظرفیت: {data.enrolled}/{data.capacity}</span>
                        <span>{seatPercentage.toFixed(0)}% پر شده</span>
                    </div>
                    <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div 
                            className={`h-full rounded-full transition-all ${
                                seatPercentage >= 80 ? 'bg-red-500' : 
